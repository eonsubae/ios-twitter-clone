...
//
//  Created by KRITSSEAN on 2021/03/03.
//
    ...
    func fetchUserStats() {
        ...
        followersRef.getDocuments { snapshot, _ in
            ...
            followingRef.getDocuments { snapshot, _ in
                guard let follwingCount = snapshot?.documents.count else { return }
                
                self.user.stats = UserStats(followers: followerCount, following: follwingCount)
            }
        }
    }
}
